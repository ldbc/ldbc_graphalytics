...
public class UndirectedForestFireModelJob extends EdgeListVertex<VIntWritable, Text, VIntWritable, Text> implements Tool {
    private static final Logger LOG = Logger.getLogger(UndirectedForestFireModelJob.class);
    ...
    /* GLOBAL HELPERS */
    private static List<VIntWritable> workerIDs = new ArrayList<VIntWritable>();
    ...
    */
    ...
    public void compute(Iterator<Text> msgIterator) throws IOException {
        ...
        if(this.getSuperstep() == 0)
            UndirectedForestFireModelJob.workerIDs.add(this.getVertexId());

        /*******************************
         Start Evolution Algorithm
         *******************************/
        // create new vertices
        if(this.getSuperstep() == 0 && !UndirectedForestFireModelJob.isNewVerticesInit) {
            UndirectedForestFireModelJob.isNewVerticesInit = true;
            UndirectedForestFireModelJob.workerNewVerticesSize = this.getWorkerNewVertices();

            int workerID = this.getWorkerId(this.getContext().getTaskAttemptID().getTaskID());
            for(int i=0; i<UndirectedForestFireModelJob.workerNewVerticesSize; i++) {
                int newID = workerID * UndirectedForestFireModelJob.workerNewVerticesSize + i + this.getMaxId();
                newID -= (UndirectedForestFireModelJob.workerNewVerticesSize - 1); // THIS IS SO LAME (prevents overwriting MAX_ID vertex)
                this.createNewVertex(newID);
            }

            return;
        } else if(this.getSuperstep() == 0 && !UndirectedForestFireModelJob.isNewVerticesInit)
            return;

        // NEW VERTICES
        if(this.getSuperstep() > 0 && this.getVertexId().get() > this.getMaxId()) {
            // initialize burning
            if(!this.isAmbassadorInit) {
                ...
                this.addEdgeRequest(ambassador, new Edge<VIntWritable, VIntWritable>(this.getVertexId(), new VIntWritable(0)));
                // statistics
            ...
            // process reply
                ...
                while (msgIterator.hasNext()) {
                    ...
                    // ambassador reply
                        ...
                        List<VIntWritable> burnOutDst = this.burnEdgesTo((int)x, neighbours);
        ...
        else if(this.getSuperstep() > 0 && this.getVertexId().get() <= this.getMaxId()) {
     ...
     */
    private List<VIntWritable> burnEdgesTo(int x, List<VIntWritable> neighbours) throws IOException {
        ...
        if(neighbours.size() <= x) {
            for(VIntWritable id : neighbours) {
                ...
                this.addEdgeRequest(id, new Edge<VIntWritable, VIntWritable>(this.getVertexId(), new VIntWritable(0)));
        ...
        // burn == X
            ...
            for(int i=x; i>0; i--) {
                ...
                this.addEdgeRequest(neighbourID, new Edge<VIntWritable, VIntWritable>(this.getVertexId(), new VIntWritable(0)));
    ...
    /*
        HELPERS
     */
    private String verticesIDsList2String(List<VIntWritable> list) {
    ...
    private void createNewVertex(int newID) throws IOException {
        ...
        UndirectedForestFireModelJob.workerNewVertices.put(newVertexID, basicVertex);
    }

    private VIntWritable randomInitAmbassador() {
        int index =  rnd.nextInt(UndirectedForestFireModelJob.workerIDs.size());
        return  UndirectedForestFireModelJob.workerIDs.get(index);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        Preconditions.checkArgument(args.length == 9,
                "Job run: Must have 9 arguments <type> <input path> <output path> <# of workers> <maxID> <nrOfNewVertices> <forward burning probability p>" +
