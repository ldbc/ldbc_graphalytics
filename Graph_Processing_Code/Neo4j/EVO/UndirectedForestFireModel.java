 ...
 */
public class UndirectedForestFireModel {
    ...
    public int run(GraphDatabaseService graphDb, Neo4Job job) throws IOException {
        ...
        for(int  i=0; i<job.getNewVertices(); i++) {
            ...
            Node newVertex = this.runFFM(job.getpRation(), job.getHoops(), ++maxID);
    ...
    protected Node runFFM(float p, int hoopsThreshold, int maxID) {
        ...
        do {
            nextStepAmbassadors.addAll(this.burn(newVertex, currentStepAmbassadors,p));
    ...
    protected List<Node> burn(Node newVertex, List<Node> ambassadors, float p) {
