...
#start daemons of mpi
let "nodesNr-=1"
MPI_MASTER=`head -1 $SCRIPTS/machines`
echo "new nodesNr" $nodesNr
echo "mpi first node" $MPI_MASTER
ssh $USER@$MPI_MASTER 'bash -s' < initMPI.sh $nodesNr $SCRIPTS/machines
