...
do
	...
	bash bfs.sh ../biggraph.jar eu.stratosphere.pact.example.biggraph.BreadthFirstSearch 120 hdfs://$nodename.cm.cluster:54310/local/hadoop.tmp.yongguo/DotaLeague_FCF hdfs://$nodename.cm.cluster:54310/local/hadoop.tmp.yongguo/output_$i\_DotaLeague_FCF 0 u DotaLeague_FCF_vs6 $i
	echo "--- Copy output ---"
...
do
	...
	bash bfs.sh ../biggraph.jar eu.stratosphere.pact.example.biggraph.BreadthFirstSearch 120 hdfs://$nodename.cm.cluster:54310/local/hadoop.tmp.yongguo/Friendster_FCF hdfs://$nodename.cm.cluster:54310/local/hadoop.tmp.yongguo/output_$i\_Friendster_FCF 71768986 u Friendster_FCF_vs6 $i
	echo "--- Copy output ---"
